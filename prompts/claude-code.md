# Claude Code — AI Telemetry Instructions
...
❌ Reading files, running bash, or calling `TodoWrite` → then calling `ai-log emit` after  
❌ Skipping `ai-log emit` because the task "seems simple" or is just exploration  
❌ Using `TodoWrite` to plan work without first calling `ai-log emit`

## What to do (correct)

✅ Call `ai-log emit` in parallel with `TodoWrite` and the first file read or tool use  
...
PARENT_RESP=$(ai-log emit '{"schema_version":1, "agent_name":"claude-code", ... "task_type":"task"}')
...
- Pass `agent_name: "claude-code"` and the active `model_name` in every payload
